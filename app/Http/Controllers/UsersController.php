...
use App\Models\{User, Education, SalaryHistory, DesignationHistory};
...
class UsersController extends Controller
{
    ...
    {
        try {
            $user = User::where('username', $request->input('username'))->with('role:id,name')->first();
    ...
    public function getAllUser(Request $request): JsonResponse
    {
        if ($request->query('query') === 'all') {
            ...
            try {
                ...
                $allUser = User::where(function ($query) use ($key) {
                ...
                $allUserCount = User::where(function ($query) use ($key) {
            ...
            try {
                ...
                $allUser = User::when('status', function ($query) use ($request) {
                ...
                $totalUser = User::when('status', function ($query) use ($request) {
                    return $query->whereIn('status', explode(',', $request->query('status')));
                })
                    ->count();
    ...
    public function getSingleUser(Request $request): JsonResponse
    {
        try {
            ...
            $singleUser = User::where('id', $request['id'])
